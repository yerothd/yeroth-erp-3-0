		...
		lister_les_elements_du_tableau(YerothSqlTableModel &aSqlTableModel)
{
    ...
    for (int i = 0; i < rows; ++i)
    {
        for (int k = 0; k < columns; ++k)
        {
            ...
            case QVariant::Int:
                if (YerothUtils::isEqualCaseInsensitive(curTableModelRawHdr, YerothDatabaseTableColumn::ROLE))
                {
                	anItem = new YerothQStandardItem(YerothUtils::_roleToUserViewString.value(qv.toInt()));
                }
                else if(YerothUtils::isEqualCaseInsensitive(curTableModelRawHdr, YerothDatabaseTableColumn::TITRE))
                {
                	anItem = new YerothQStandardItem(YerothUtils::_titreToUserViewString.value(qv.toInt()));
                }
