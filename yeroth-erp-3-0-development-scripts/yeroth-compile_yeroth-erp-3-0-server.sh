...
if [[ "${@}" == *"s"* || "${@}" == *"h"* ]]; then
		yeroth-compile_yeroth-erp-3-0.sh -v YEROTH_SERVER "${@}"
		exit
fi

if [[ "${@}" == *"e"* ]]; then
	OUT_CMD_TIME=$(time yeroth-compile_yeroth-erp-3-0.sh -v YEROTH_SERVER "${@}" > /dev/null 2>&1) && \
...
val=$(cat YEROTH_SERVER.error.log | grep 'error\|ERROR\|Error' | wc -l)
